...
const ALERT_EMAIL = process.env.ADMIN_EMAIL || process.env.ALERT_EMAIL || '';
const RESEND_API_KEY = process.env.RESEND_API_KEY || '';
const SITE_NAME = process.env.SITE_NAME || 'Athlete Spotlight';
const FROM_EMAIL = process.env.FROM_EMAIL || 'Athlete Website <onboarding@resend.dev>';
...
async function sendAlert(lead) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: FROM_EMAIL,
      to: [ALERT_EMAIL],
      subject: `[${SITE_NAME}] New ${lead.leadType}`,
      html: `
